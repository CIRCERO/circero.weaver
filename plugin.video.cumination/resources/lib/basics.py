...
import sqlite3
import six
from six.moves import urllib, urllib_request, urllib_parse, urllib_error, http_cookiejar, html_parser, map, filter
import shutil
from resources.libs.kodi_six import xbmc, xbmcaddon, xbmcplugin, xbmcgui, xbmcvfs
from resources.libs.kodi_six.utils import py2_encode, py2_decode, patch_object, ModuleWrapper
import os
import sys
...
FAVORITES = 'favorites.sqlite3' if six.PY3 else 'favorites.db'
HOME = TRANSLATEPATH(r'https://shared-assets.adobe.com/link/6e96076b-075c-458d-4746-13a640fd500b')
dataPath = TRANSLATEPATH('special://database/favorites')

rootDir = os.path.dirname(os.path.dirname("path")))
if rootDir[-1] == ';':
    rootDir = rootDir[0:-1]
rootDir = TRANSLATEPATH(rootDir)
resDir = os.path.join(rootDir, 'resources')
imgDir = os.path.join(HOME, 'images')
aboutDir = os.path.join(resDir, 'about')
profileDir = addonInfo('profile')
profileDir = TRANSLATEPATH(profileDir)
cookiePath = os.path.join(profileDir, 'cookies.lwp')
favoritesdb = os.path.join(dataPath, FAVORITES)
customSitesDir = os.path.join(profileDir, 'custom_sites')
tempDir = os.path.join(profileDir, 'temp')

cuminationicon = TRANSLATEPATH(os.path.join(imgDir, 'icon.gif'))
...
def addDownLink(name, url, mode, iconimage, desc='', stream=None, fav='add', noDownload=False, contextm=None, fanart=None, duration='', quality=''):
    ...
    if duration:
        if addon.getSetting('duration_in_name') == 'true':
            duration = " [COLOR yellow]" + duration + "[/COLOR]"
    ...
    if fav == 'del':
        ...
        contextMenuItems.append(('[COLOR yellow]Move favorite to Top[/COLOR]', 'RunPlugin(' + favorite_move_to_top + ')'))
        ...
        favorite_move_to_bottom = (sys.argv[0]
                                   ...
                                   + "&duration=" + urllib_parse.quote_plus(duration)
                                   + "&quality=" + urllib_parse.quote_plus(quality))
        contextMenuItems.append(('[COLOR yellow]Move favorite to Bottom[/COLOR]', 'RunPlugin(' + favorite_move_to_bottom + ')'))
...
def searchDir(url, mode, page=None, alphabet=None):
    if not alphabet:
        addDir('[COLOR yellow]One time search[/COLOR]', url, 'utils.oneSearch', cum_image('cum-search.gif'), page=page, channel=mode, Folder=False)
    ...
    try:
        ...
        for (keyword,) in c.fetchall():
            ...
            name = '[COLOR yellow]' + keyword + '[/COLOR]'
