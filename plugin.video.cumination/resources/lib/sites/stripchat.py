...
site = AdultSite('stripchat', '[COLOR crimson]stripchat.com[/COLOR]', 'http://stripchat.com/', 'stripchat.jpg', 'stripchat', True)
...
def Main():
    ...
    site.add_dir('[COLOR crimson]HD[/COLOR]', '{0}hd'.format(bu), 'List', '', '')
    if female:
        site.add_dir('[COLOR crimson]Female[/COLOR]', '{0}female'.format(bu), 'List', '', '')
    if couple:
        site.add_dir('[COLOR crimson]Couples[/COLOR]', '{0}couples'.format(bu), 'List', '', '')
    if male:
        site.add_dir('[COLOR crimson]Male[/COLOR]', '{0}men'.format(bu), 'List', '', '')
    if trans:
        site.add_dir('[COLOR crimson]Transsexual[/COLOR]', '{0}trans'.format(bu), 'List', '', '')
    utils.eod()
...
def List2(url):
    ...
    for url, img, name in match:
        if 'profile' in url:
            name = '[COLOR crimson][Offline][/COLOR] ' + name
            url = "  "
