...
site = AdultSite('luxuretv', '[COLOR crimson]LuxureTV[/COLOR]', 'https://luxuretv.com/', 'luxuretv.gif', 'luxuretv')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR crimson]Categories[/COLOR]', site.url + 'channels/', 'Cat', site.img_cat)
    site.add_dir('[COLOR crimson]Search[/COLOR]', site.url + 'searchgate/videos/', 'Search', site.img_search)
    List(site.url)
...
def List(url):
    ...
    if np:
        ...
        site.add_dir('[COLOR crimson]Next Page...[/COLOR] ({0})'.format(np.split('page')[-1].split('.')[0]), np, 'List', site.img_next)
