...
site = AdultSite('peekvids', '[COLOR crimson]PeekVids[/COLOR]', 'https://www.peekvids.com/', 'https://www.peekvids.com/img/logo.png', 'peekvids')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR crimson]Categories[/COLOR]', site.url + 'categories', 'Cat', site.img_cat)
    site.add_dir('[COLOR crimson]Channels[/COLOR]', site.url + 'channels', 'Channels', site.img_cat)
    site.add_dir('[COLOR crimson]Search[/COLOR]', site.url + 'sq?q=', 'Search', site.img_search)
...
def List(url):
    ...
    if np:
        ...
        site.add_dir('[COLOR crimson]Next Page...[/COLOR] ({0})'.format(np.split('=')[-1]), np, 'List', site.img_next)
...
def Cat(url):
    ...
    for catpage, name, vids in match:
        name += ' [COLOR crimson]{0} Videos[/COLOR]'.format(vids)
...
def Channels(url):
    ...
    if np:
        ...
        site.add_dir('[COLOR crimson]Next Page...[/COLOR] ({0})'.format(np.group(2)), nextpg, 'Channels', site.img_next)
