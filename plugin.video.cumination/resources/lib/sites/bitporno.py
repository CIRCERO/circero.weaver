...
site = AdultSite('bitporno', '[COLOR crimson]Bitporno[/COLOR]', 'https://www.bitporno.com/', 'https://www.bitporno.com/images/logobt.png', 'bitporno')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR crimson]Categories[/COLOR]', site.url, 'Cat', site.img_cat)
    site.add_dir('[COLOR crimson]Search[/COLOR]', site.url + '?q=', 'Search', site.img_search)
...
def List(url):
    ...
    if match:
        npage, np = match[0]
        site.add_dir('[COLOR crimson]Next Page...[/COLOR] ({0})'.format(np), site.url[:-1] + npage, 'List', site.img_next)
    utils.eod()
