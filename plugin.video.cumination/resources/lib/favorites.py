...
try:
    ...
    c.executescript("CREATE TABLE IF NOT EXISTS mapping (favorites TEXT UNIQUE, media_folder TEXT, favorites_type TEXT, media_type TEXT, kodi_id INTEGER, kodi_fileid INTEGER, kodi_pathid INTEGER, parent_id INTEGER, favorites_parent_id TEXT, presentation_key TEXT);")
    ...
    c.executescript("CREATE TABLE IF NOT EXISTS MediaSources (favorites TEXT, MediaIndex INTEGER, MediaSourceId TEXT, Path TEXT, Name TEXT, Size INTEGER);")
...
class Database():
    def __init__(self, Utils, fileID, commit_close):
        ...
        if fileID.startswith('favorites'):
            self.DBFile = self.Utils.DatabaseFiles['favorites'] + fileID + ".sqlite3" if six.PY3 else ".db"
        else:
            self.DBFile = self.Utils.DatabaseFiles[self.db_file]

    def __enter__(self):
        self.conn = sqlite3.connect(self.DBFile, timeout=120)
        self.c = self.conn.cursor()
        self.c.execute("PRAGMA journal_mode=WAL") # to avoid writing conflict with kodi
